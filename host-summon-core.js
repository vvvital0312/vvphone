    (function () {
      const VVHOST_VERSION = 'SUMMON-CALL-INTERCEPT-001';
      const PHONE_ORIGIN = 'https://vvvital0312.github.io';
      ...
      console.log('[VVHOST][SUMMON] loaded', location.href, 'version:', VVHOST_VERSION);
      ...
      function getCtx() {
        var root = getRoot();
        try { return root.SillyTavern.getContext(); } catch (e) { return null; }
      }

      function postToPhone(payload) {
        try {
          if (!phoneFrame || !phoneFrame.contentWindow) {
            console.warn('[VVHOST][SUMMON] phoneFrame not ready');
            return false;
          }
          phoneFrame.contentWindow.postMessage(payload, PHONE_ORIGIN);
      ...
      function postFeedSyncToPhone(block, viewId) {
        ...
        if (!phoneFrame || !phoneFrame.contentWindow) {
          console.warn('[VVHOST] postFeedSyncToPhone: phoneFrame not found');
          return;
        }
        ...
        console.log('[VVHOST] posted VVPHONE_FEED_SYNC to phone, length=', block.length);
      }
      ...
      window.addEventListener('message', async function (event) {
        ...
        try {
          ...
          if (data.type === 'VV_REQUEST_FEED_REFRESH') {
            var ctxF = SillyTavern.getContext();
            for (var fi = 0; fi < ctxF.chat.length; fi++) {
              if (ctxF.chat[fi].mes && ctxF.chat[fi].mes.includes('V_FEED_HIDDEN_DATA')) {
                postToPhone({ type: 'VV_FEED_HIDDEN_RAW', raw: ctxF.chat[fi].mes });
                console.log('[VHOST] feed refresh sent, length:', ctxF.chat[fi].mes.length);
                break;
              }
            }
          }
