(function () {
    ...
    try {
      if (data.type === 'VVPHONE_READY') {
        ...
        try {
          var chat = getSTChat();
          if (chat && chat.length > 0) {
            // 从最后一条 assistant 消息里找 VV_CHAT_SYNC
            for (var i = chat.length - 1; i >= 0; i--) {
              var msg = chat[i];
              if (!msg || msg.is_user) continue;
              var text = String(msg.mes || msg.message || '');
              var syncBlock = extractValidVVChatSyncBlock(text, CURRENT_CHAT_ID);
              if (syncBlock) {
                console.log('[VVHOST_CHAT] INIT: found sync block at index=', i, ', posting to phone');
                postChatSyncToPhone(syncBlock, CURRENT_CHAT_ID, lastViewId || '');
                break;
              }
              // 只查最近3条，避免扫太远
      ...
      if (data.type === 'VV_EXECUTE_SLASH') {
        ...
        try {
          ...
          if (feedMode) {
            ...
            console.log('[VVHOST_FEED] call mode active, skip polling');
          } else {
  ...
  (function initFeedHiddenDataWatcher() {
    ...
    console.log('[VVHOST_FEED] feed hidden data watcher started');
  })();
